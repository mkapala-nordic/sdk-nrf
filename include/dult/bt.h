...
/** Maximum length of the proprietary data when the surrounding AD payload
 *  includes the optional Flags TLV (default, spec-compliant case).
 *
 ...
 */
#define DULT_BT_ADV_PROPRIETARY_DATA_MAX_LEN_WITH_FLAGS	\
	(DULT_BT_ADV_PROPRIETARY_DATA_SPEC_MAX_LEN)

/** Maximum length of the proprietary data when the surrounding AD payload
 *  omits the optional Flags TLV.
 *
 ...
 *  emitted in the surrounding AD payload. The caller is responsible for
 *  enforcing this precondition.
 */
#define DULT_BT_ADV_PROPRIETARY_DATA_MAX_LEN_NO_FLAGS	\
	(DULT_BT_ADV_PROPRIETARY_DATA_SPEC_MAX_LEN + DULT_BT_ADV_FLAGS_TLV_LEN)

/** Absolute maximum length of the proprietary data accepted by
 *  @ref dult_bt_adv_data_fill in bytes.
 *
 *  Equal to @ref DULT_BT_ADV_PROPRIETARY_DATA_MAX_LEN_NO_FLAGS. Integrators
 *  that include a Flags TLV in their AD payload MUST cap their proprietary
 *  payload at @ref DULT_BT_ADV_PROPRIETARY_DATA_MAX_LEN_WITH_FLAGS instead.
 */
...
/** @brief Initialize the DULT Bluetooth advertising data structure
 *         with no proprietary payload.
 *
 *  Convenience initializer for advertising flows that do not append any
 *  network-specific proprietary data.
 *
 *  @param[in] _near_owner Near-owner state.
 *
 *  @return Initialized DULT Bluetooth advertising data structure.
 */
#define DULT_BT_ADV_DATA_INIT_NO_PROPRIETARY(_near_owner)	\
	{							\
		.near_owner = (_near_owner),			\
		.proprietary = {				\
			.buf = NULL,				\
