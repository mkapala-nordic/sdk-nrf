...
The system can be configured to use the MCUboot in the direct-xip mode in the split slot configuration by building the sample with the ``split_slot`` file suffix.
...
.. tabs::
   ...
   .. group-tab:: Merged slot configuration
      ...
      #. The whole Radio TCM RAM region is used for the remote firmware (:file:`common/memory_map_ram_cpurad.dtsi`).
      #. The small piece of Global RAM is used for the radio loader for the firmware relocation time (:file:`common/memory_map.dtsi`).
   ...
   .. group-tab:: Split slot configuration
      ...
      #. The whole Radio TCM RAM region is used for the remote firmware (:file:`common/memory_map_ram_cpurad.dtsi`).
      #. The small piece of Global RAM is used for the radio loader for the firmware relocation time (:file:`common/memory_map_split_slot.dtsi`).
      #. The MRAM is partitioned for the MCUboot, application and radio core images (:file:`common/memory_map_split_slot.dtsi`).
      #. The ``pm_ramfunc`` DTS node is relocated to the end of the RAM block but before the MCUboot trailer (:file:`common/memory_map_ram_pm_cpurad.dtsi`) so the image properly links.
...
You must provide the DTS overlay file for the radio loader that includes the following DTS nodelabels:
...
Firmware relocation is handled automatically by Zephyr's build system using the ``CONFIG_BUILD_OUTPUT_ADJUST_LMA`` configuration option, which is configured in ``nrf/soc/nordic/nrf54h/Kconfig.defconfig.nrf54h20_cpurad`` for all nRF54H20 CPURAD projects.
...
When this option is disabled, the remote firmware runs from the ``zephyr,sram`` location but the code to be loaded into TCM is stored in the MRAM in the ``zephyr,code-partition`` location.
The build system calculates the LMA adjustment to relocate the firmware from MRAM to TCM.
...
To use the radio loader pattern, you must to configure the remote radio firmware in the following way:
         ...
         * For the application core, the output should be as follows:
         ...
         .. code-block:: console
            ...
            *** Using Zephyr OS v4.2.99-56fbb4f3c7bb ***
            ...
            [00:00:00.336,740] <inf> idle: Current PC (program counter) address: 0xe041290
         ...
         * For the application core, the output should be as follows:
         ...
         .. code-block:: console
            ...
            *** Using Zephyr OS v4.2.99-56fbb4f3c7bb ***
            ...
            [00:00:00.116,193] <inf> idle: Current PC (program counter) address: 0xe041290
