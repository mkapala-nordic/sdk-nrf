...
static int do_udp_server_start(uint16_t port)
{
	...
	if (proxy.sec_tag != INVALID_SEC_TAG) {
		...
		ret = zsock_setsockopt(proxy.sock, SOL_TLS, TLS_NATIVE,
		...
		ret = zsock_setsockopt(proxy.sock, SOL_TLS, TLS_DTLS_ROLE,
				       &tls_role, sizeof(int));
		if (ret) {
	...
	ret = zsock_setsockopt(proxy.sock, SOL_SOCKET, SO_REUSEADDR,
...
static int do_udp_client_connect(const char *url, uint16_t port)
{
	...
	/* Open socket */
	ret = zsock_socket(proxy.family, SOCK_DGRAM,
			   using_dtls ? IPPROTO_DTLS_1_2 : IPPROTO_UDP);
	if (ret < 0) {
		LOG_ERR("socket() failed: %d", -errno);
		return -errno;
	}
	...
	if (using_dtls) {
		...
		ret = zsock_setsockopt(proxy.sock, SOL_SOCKET, SO_SNDTIMEO,
		...
		ret = zsock_setsockopt(proxy.sock, SOL_TLS, TLS_PEER_VERIFY,
				       &proxy.peer_verify, sizeof(proxy.peer_verify));
		if (ret) {
		...
		if (proxy.hostname_verify) {
			ret = zsock_setsockopt(proxy.sock, SOL_TLS, TLS_HOSTNAME,
					       url, strlen(url));
		} else {
			ret = zsock_setsockopt(proxy.sock, SOL_TLS, TLS_HOSTNAME,
					       NULL, 0);
...
static void udp_thread_func(void *p1, void *p2, void *p3)
{
	...
	do {
		ret = zsock_poll(fds, ARRAY_SIZE(fds),
				 MSEC_PER_SEC * CONFIG_SLM_UDP_POLL_TIME);
		if (ret < 0) {  /* IO error */
		...
		if ((fds[SOCK].revents & ZSOCK_POLLERR) != 0) {
			...
			ret = zsock_getsockopt(proxy.sock, SOL_SOCKET, SO_ERROR,
					       &value, &len);
			if (ret) {
				LOG_ERR("%d : zsock_getsockopt(SO_ERROR) error: %d",
					fds[SOCK].fd, -errno);
				ret = -EIO;
				break;
			}
