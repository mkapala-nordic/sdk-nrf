...
if(CONFIG_NCS_IS_VARIANT_IMAGE)
  ...
  if(SB_CONFIG_PARTITION_MANAGER)
    ...
    dt_partition_addr(code_partition_offset PATH "${code_partition}" REQUIRED) # ADD ABSOLUTE
    ...
    foreach(line IN LISTS dotconfig_content)
      ...
      if("${line}" MATCHES "^CONFIG_BUILD_OUTPUT_ADJUST_LMA=.*$")
        ...
        dt_reg_addr(sram_offset PATH "${sram_property}" REQUIRED)
        set(flash_base_addr 0xe000000)
        set(adjusted_lma "${flash_base_addr}+${code_partition_offset}-${sram_offset}")
        string(REGEX REPLACE "CONFIG_BUILD_OUTPUT_ADJUST_LMA=(.*)" "CONFIG_BUILD_OUTPUT_ADJUST_LMA=\"${adjusted_lma}\"" line ${line})
      endif()
    ...
    foreach(line IN LISTS autoconf_content)
      ...
      if("${line}" MATCHES "^#define CONFIG_BUILD_OUTPUT_ADJUST_LMA .*$")
        ...
        set(flash_base_addr 0xe000000)
        set(adjusted_lma "${flash_base_addr}+${code_partition_offset}-${sram_offset}")
        string(REGEX REPLACE "#define CONFIG_BUILD_OUTPUT_ADJUST_LMA (.*)" "#define CONFIG_BUILD_OUTPUT_ADJUST_LMA \"${adjusted_lma}\"" line ${line})
      endif()
